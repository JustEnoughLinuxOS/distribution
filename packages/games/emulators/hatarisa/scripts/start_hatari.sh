...
if [ `echo $1 | grep -i .zip | wc -l` -eq 1 ]; then
  ...
  do
    ...
    nice -n -19 hatari --configfile "$HATARI_TMP_CONFIG" >> $HATARI_LOG 2>&1

# Check for .cfg config file
elif [ `echo $1 | grep -i .cfg | wc -l` -eq 1 ]; then
  nice -n -19 hatari --configfile "$1" >> $HATARI_LOG 2>&1
# All other files
else
  nice -n -19 hatari --configfile "$HATARI_TMP_CONFIG" --disk-a "$1" >> $HATARI_LOG 2>&1
fi
